...
public class BindingReflectionUtils {
	...
	public static boolean isTBComponent(IType type, TypeCache cache) throws JavaModelException {
		return BindingReflectionUtils.isType(type, new String[] { "org.treasureboat.webcore.components.TBComponent" }, cache);
	}

	public static boolean isTBFKeyValueCoding(IType type, TypeCache cache) throws JavaModelException {
		return BindingReflectionUtils.isType(type, new String[] { "org.treasureboat.foundation.kv.TBFKeyValueCoding" }, cache);
	}

	public static boolean isTBFCollection(IType type, TypeCache cache) throws JavaModelException {
		return BindingReflectionUtils.isType(type, new String[] { "org.treasureboat.foundation.dic.TBFDictionary", "org.treasureboat.foundation.array.TBFArray", "org.treasureboat.foundation.TBFSet", "org.treasureboat.webcore.localization.TBWLocalizer" }, cache);
	}
	...
	public static List<BindingValueKey> getBindingKeys(IJavaProject javaProject, IType type, String nameStartingWith, boolean requireExactNameMatch, int accessorsOrMutators, boolean allowInheritanceDuplicates, TypeCache cache) throws JavaModelException {
		...
		if (type != null) {
			...
			// We want to show fields from your TBApplication, TBSession, and
			// TBDirectAction subclasses ...
			...
			if (types != null) {
				...
				for (int typeNum = 0; usuallySubclassedSupertype == null && typeNum < types.size(); typeNum++) {
					...
					if ("TBApplication".equals(typeName) || "TBSession".equals(typeName) || "TBDirectAction".equals(typeName)) {
						usuallySubclassedSupertype = types.get(typeNum);
					}
					else if ("TBFArray".equals(typeName)) {
						for (String operator : BindingReflectionUtils.getArrayOperators()) {
							additionalProposals.add("@" + operator);
						}
						//						IType nextType = types.get(typeNum);
					}
				}
	...
	static {
		...
		_systemTypeNames.add("TBElement");
		_systemTypeNames.add("ITBWActionResults");
		_systemTypeNames.add("TBComponent");
