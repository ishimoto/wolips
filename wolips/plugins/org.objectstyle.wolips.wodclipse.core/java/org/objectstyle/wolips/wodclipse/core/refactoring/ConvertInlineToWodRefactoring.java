      ...
      if (element != null) {
        ...
        if (WodHtmlUtils.isInline(tagName)) {
          ...
          if (element.hasCloseTag()) {
            edits.add(new ReplaceEdit(element.getCloseTagOffset() + element.getCloseNameOffset() + 1, element.getCloseNameLength(), "treasureboat"));
          }
          else {
            openTagLength--;
          }
          edits.add(new ReplaceEdit(element.getOffset() + 1, openTagLength, "treasureboat name = \"" + elementRename.getNewName() + "\""));
